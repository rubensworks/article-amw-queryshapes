...
after which we define how an FSS can be matched to a SPARQL query.
Afterwards, we demonstrate the expressivity of FSSL by expressing all well known LDF languages using FSSs.

### Grammar

The following expressions build upon references to [algebraic SPARQL 1.2 query expressions](spec:sparqlquery12),
...
$$
\begin{array}{ll}
fss  & = & fsse \ | \ fssa \ | \ (fss \land fss) \ | \ (fss \lor fss) \ | \ \neg (fss) \ | \ \epsilon \\
fsse & = & \langle e, A_{scope}, fss \rangle_e \\
fssa & = & \langle fss, n_1, n_2 \rangle_a
\end{array}
$$
...
considers the arguments $$ A_{scope} \subseteq ( \mathcal{V} \cup \mathcal{I} \cup \mathcal{B} \cup \mathcal{L} \cup \mathcal{N} ) $$ in scope for this operation,
together with a child shape.
...
this means that only concrete RDF terms ($$ \mathcal{I} \cup \mathcal{B} \cup \mathcal{L} \cup \mathcal{N} $$) can be filled into this position.
Arguments written as $$ \_ $$ mean that no values can be provided for them.
...
where the first natural number $$ n_1 $$ refers to the minimum required number of occurrences (inclusive),
and the second natural number $$ n_2 $$ refers to the maximum allowed number of occurrences (exclusive).
...
Within this definition, we informally consider the arguments in $$ A_{scope} $$ to propagate down into its child shapes.
This means that if an argument $$ a $$ occurs at any point within an $$ fss $$, and it is matched to a certain RDF term $$ t $$,
then all of the direct or indirect children of $$ fss $$ that contain this argument $$ a $$ must match to this RDF term $$ t $$ within the query expression that is considered at this level or deeper.

### LDF Language Instantiations

In listing [](#ldf-fss), we instantiated the FSS for each LDF type we could find in the literature.
This demonstrates that FSSL is sufficiently expressive to represent all LDF types that are known so far.

<figure id="ldf-fss" class="listing" markdown="1">

$$
\begin{array}{ll}

fss_{SPARQLEP} & = & \epsilon \\
fss_{TPF} & = & \langle tp \rangle_e \\
fss_{QPF} & = & fss_{TPF} \ | \ \langle graph, \emptyset, fss_{TPF} \rangle_e \\
fss_{URIF} & = & \langle tp(s^*, \_, \_) \rangle_e \\
fss_{brTPF} & = & fss_{TPF} \ | \ \langle values, \emptyset, fss_{TPF} \rangle_e \\
fss_{SPF} & = & \langle bgp, \{ s \}, \langle tp(s, p, o), 1, \infty \rangle_a \rangle_e \\
fss_{SMARTKG} & = & fss_{brTPF} \ | \ \langle bgp, \emptyset, \langle tp(s, p^*, o), 1, \infty \rangle_a \rangle_e \\
fss_{WISEKG} & = & fss_{SPF} \ \mid \ fss_{SMARTKG} \\
...
fss_{VTPF} & = & \langle mat, \emptyset, fss_{TPF} \rangle_e \lor \langle diff, \emptyset, fss_{TPF} \rangle_e \lor \langle ver, \emptyset, fss_{TPF} \rangle_e \\
...
The simplest example of a FSS is for the language $$ l_{SPARQL12} $$ corresponding to a SPARQL 1.2 endpoint $$ fss_{SPARQLEP} $$.
[SaGe](cite:cites sage) also shares this language, as it can accept any SPARQL query, and is only limited by execution time.
Another straighforward example is an FSS for the language $$ l_{TP} $$ corresponding to a [Triple Pattern Fragments (TPF) interface](cite:cites tpf) $$ fss_{TPF} $$, with $$ tp $$ referring to the algebraic triple pattern expression.
As an extension, the FSS for the [Quad Pattern Fragments interface](cite:cites spec:qpf) (TPF with named graph support)
can be expressed as $$ fss_{QPF} $$, with $$ graph $$ referring to the algebraic graph query expression.
...
can be expressed as $$ fss_{URIF} $$.
[brTPF](cite:cites brtpf) is another extension to TPF that allows bindings to be pushed into triple patterns.
As such, we can express the FSS for brTPF as $$ fss_{brTPF} $$,
with $$ values $$ referring to the algebraic values query expression,
which allows solution mappings to be passed.
...
As such, we can express the FSS for SPF as $$ fss_{SPF} $$.
[Smart-KG](cite:cites smartkg) is an LDF approach that builds upon brTPF,
but also allows multiple triple patterns to be sent at once.
Unlike SPF, smart-KG does not require star patterns,
but it does not accept variables in the predicate position.
Hence, we can express the FSS for smart-KG as $$ fss_{SMARTKG} $$.
[WiseKG](cite:cites wisekg), being a combination of both SPF and smart-KG, can be expressed as $$ fss_{WISEKG} $$.
[Passage](cite:cites passage) is an LDF approach supporting what the authors call *Core SPARQL*,
which includes triple patterns, BGPs, joins, unions, optionals, filters, offsets, and binds.
Hence, we can express the FSS for Passage as $$ fss_{PASSAGE} $$.
...
For this, we express the FSS for VTPF as $$ fss_{VTPF} $$.