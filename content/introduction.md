...
One of the most popular techniques today to publish a Knowledge Graph, is through a [SPARQL endpoint](cite:cites spec:sparqlprot12).
...
For this reason, such interfaces can be compared to each other using the *Linked Data Fragments* axis as shown in [](#ldf-axis),
...
federated querying over multiple instances per interface is only introduced for some interface types.
...
Next, [a more intelligent query decomposition and query planning](cite:cites heterogeneous_lars) was introduced where more than just triple patterns can be assigned to federation members if their interface supports it.
...
The Linked Data Fragments axis shows the trade-offs between server-side and client-side effort for SPARQL query execution with different LDF interface types.
While the left-hand side requires mostly client-side effort for executing a query,
the right-hand side requires mostly server-side effort.
The axis contains all LDF interface types that have been studied in literature at the time of writing.
</figcaption>
</figure>
...
The goal of this work is close that gap by introducing a new formal language called the *Fragment Selector Shapes Language* (FSSL)
to describe *Fragment Selector Shapes* (FSSs), which represent the expressivity of LDF interfaces.
LDF interfaces can expose their expressivity as an FSS,
which can be used by clients to determine how they should decompose their query,
without requiring prior knowledge of LDF interface expressivity.
...
but they can decompose their query purely based on the server-provided FSSs.
...
In [the next section](#example), we introduce a motivating example,
followed by the necessary preliminaries in [](#preliminaries) onto which FSSL will be based.
Next, we introduce FSSL formally in [](#shapes),
after which we provide a query decomposition algorithm in [](#querydecomposition)
that can be used during source selection over federations of heterogeneous interfaces.
Finally, we conclude in [](#conclusions) with next steps for future work.
